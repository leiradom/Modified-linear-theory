...
pressure = np.loadtxt("pressuremodel12.txt",dtype=np.float64)   #存放未修正的压力信号
...
for i0 in range(0,a-1):
    if(math.sqrt((X[i0+1] - X[i0])**2 + (Y[i0+1] - Y[i0])**2) >1.3):   #1.3
...
for i in range(0,a-1):
    ...
    #先找到距离较大的点
    if(abs(X[i+1]-X[i])>0.5):   #if((li >= l0))  #不再利用线段长度作为判据，改用x方向的位移量
        px = i                       #记录每一个距离较大的地方的i
