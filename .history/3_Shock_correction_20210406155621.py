...
pressure = np.loadtxt("pressuremodel1.txt",dtype=np.float64)   #存放体积分布的数据文件
a = len(pressure)

X = np.ones(a,dtype=np.float64)     #存放x的数组
Y = np.ones(a,dtype=np.float64)     #存放y的数组
b0 = 50                             #较大的距离离散的数目
...
for i0 in range(0,a-1):
    ...
    if(math.sqrt((X[i0+1] - X[i0])**2 + (Y[i0+1] - Y[i0])**2) >1.3):
    #if((X[i+1] - X[i]) * (Y[i+1] - Y[i])):
        #px = i0                            #记录此时的i
        #print(px,X[i0])
        l00 = math.sqrt((X[i0+1] - X[i0])**2 + (Y[i0+1] - Y[i0])**2)
        l0 = l00 / 20                     #判断是否要插值的距离
        #h0 = ((X[i0+1] - X[i0])/(b0)) 
