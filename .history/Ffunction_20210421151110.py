...
r0 = 328.5                      #r0为计算的位置  DWB为63.072  model1为20 2、3为10
con = (gama * M**2)/(math.sqrt(2 * B * r0))   #最后用来求压力的
...
for i in range(1,a):
    ...
    for j in range(0,i-1):
        s0 = (Y[j+1] * (1 / math.sqrt(X[i]-X[j+1])) + Y[j] * (1 / math.sqrt(X[i]-X[j]))) \
        * (X[j + 1] - X[j]) / (2)
        s = s0 + s
    #------------------#
    F[i] = (1/(2 * math.pi)) * s
