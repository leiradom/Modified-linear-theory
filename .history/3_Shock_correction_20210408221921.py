...
pressure = np.loadtxt("pressureDWB2.txt",dtype=np.float64)   #存放未修正的压力信号
a = len(pressure)                                              #未加点时的点数

X = np.ones(a,dtype=np.float64)     #存放x的数组 用于加点用的横坐标数组
Y = np.ones(a,dtype=np.float64)     #存放y的数组 用于加点用的纵坐标数组
X1 = np.ones(a,dtype=np.float64)     #存放x的数组 用于加点用的横坐标数组
Y1 = np.ones(a,dtype=np.float64)     #存放y的数组 用于加点用的纵坐标数组
...
for i in range(0,a-1):
    ...
    #先找到距离较大的点
    #if((li >= l0)):
    #if(abs(X[i+1]-X[i])>0.01):     #不再利用线段长度作为判据，改用x方向的位移量
        #px = i                       #记录每一个距离较大的地方的i
    ...
    for j in range(1,b0):
        ...
        #将增加的点添加到数组内
    for k in range(1,b0):
        X1 = np.insert(X,i+k,x0[k])
        Y1 = np.insert(Y,i+k,y0[k])
    a = a + 49
    print(a)
    p0 = i
